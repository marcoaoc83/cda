...
class GraficosParcelasController extends Controller
{
    public function parcelas(Request $request){
        ...
        $parc=Parcela::select(DB::raw('Year(cda_parcela.VencimentoDt) as ano,  SUM(cda_parcela.TotalVr) as total'))
            ->leftJoin('cda_pcrot','cda_parcela.ParcelaId','=','cda_pcrot.ParcelaId')
            ->leftJoin('cda_roteiro','cda_roteiro.RoteiroId','=','cda_pcrot.RoteiroId')
            ->whereRaw($where)
            ->where('cda_parcela.SitPagId', '61')
            ->groupBy(DB::raw('Year(cda_parcela.VencimentoDt)'));


        $arr=[];
        foreach ($parc->cursor() as $value){
            $arr[$value->ano]=number_format($value->total,0,'','');
        }
        $res=[];
        $x=0;
