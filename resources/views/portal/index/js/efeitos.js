...
function arrumaTela(){
	var tela = $(window).outerHeight();
	var topo = $('.pf-topo-site').outerHeight();
	var conteudo = $('#PF').outerHeight();
	var rodape = $('.pf-footer').outerHeight();
	if(tela > conteudo){
		$('#PF').css('min-height', tela);
		$('.pf-main').css('min-height', (tela - rodape) - topo);
	}
}

$(window).ready(function(){arrumaTela()}); 

$(function(){	
	var options = { onKeyPress: function (cpf, ev, el, op) { var masks = ['000.000.000-000', '00.000.000/0000-00'], mascara = (cpf.length > 14) ? masks[1] : masks[0]; $('.cpfOuCnpj').mask(mascara, op); } }	
	...
	if(isEdge || isIE){
		$('#PF').append('<link rel="stylesheet" type="text/css" href="css/css-ie.css">');
	}
